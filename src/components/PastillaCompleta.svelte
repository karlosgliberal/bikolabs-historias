<script>
  ...
  import { vis, init } from '$lib/store.js'
  ...
  <div class="w-full h-full z-20 wrapper_{position} screen_{id}">
    ...
    >
      {#if position == 'center'}
        {#if titulo != ''}
          {#if id == '1'}
            <h2 class="text-2xl tablet:text-5xl font-cabin font-bold pl-4 tablet:pl-7 mb-1 tablet:mb-3">
              <Titulo {titulo} />
            </h2>
          {/if}
          {#if id != '1'}
            <h2 class="text-2xl tablet:text-4xl font-cabin font-bold pl-4 tablet:pl-7">
              <Titulo {titulo} />
            </h2>
          {/if}
        {/if}
          ...
          {#if id == '28'}
            <div class="mt-6 flex justify-center">
              <a
                on:click={increment}
                href="/"
                class="text-sm border uppercase py-3 px-3 mr-3 flex items-center gap-3 hover:bg-white hover:text-black
            "
                >¿Volver a ver?
                <svg
